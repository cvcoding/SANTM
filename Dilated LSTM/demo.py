...
unrolled_dim = 256  # 768  # cifar data input (img shape: 28*28) # timesteps
num_of_layers = 3
num_input = 3
number_of_classes = 10  # MNIST total classes (0-9 digits)
cell_type_list = ["LSTM", "VanillaRNN"]
hidden_units = 32  # hidden layer num of features
...
data_list = load_data('cifar10', seq_len=200)
...
train_dataset = tf.data.Dataset \
            ...
            .shuffle(buffer_size=1*batch_size) \
...
for permutation in permutation_list:
    ...
    for cell_type in cell_type_list:
        ...
        # total_params = sum(p.numel() for p in pred_object.parameters())
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(1, number_of_epochs+1):
                ...
                sess.run(train_iterator, feed_dict={X: X_train, y: y_train, batch_size: 64})
                ...
                sess.run(val_iterator, feed_dict={ X: X_test, y: y_test, batch_size: len(y_test) })
                test_loss, test_accuracy = sess.run([loss_func, accuracy])
                results_val_set.append((test_loss, test_accuracy))
                ...
                if epoch % 1 == 0 or epoch == 1:
                    ...
                    print("Epoch " + str(epoch) + ", Training Loss= " + \
                          ...
                          + ", Test Loss= " + \
                          "{:.4f}".format(test_loss) + ", Validation Accuracy= " + \
