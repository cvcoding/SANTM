...
import rnn_cell_impl_local, rnn_cell_impl_doublelocal
...
def bidirectional_dynamic_rnn(cell_fw, cell_bw, inputs, sequence_length=None,
  ...
  """
  rnn_cell_impl_local.assert_like_rnncell("cell_fw", cell_fw)
  rnn_cell_impl_local.assert_like_rnncell("cell_bw", cell_bw)
...
def dynamic_rnn(cell, inputs, sequence_length=None, initial_state=None,
  ...
  """
  rnn_cell_impl_doublelocal.assert_like_rnncell("cell", cell)
...
def _dynamic_rnn_loop(cell,
  ...
  hidden_size = state_size[0]

  flat_input = nest.flatten(inputs)
  flat_output_size = nest.flatten(cell.output_size)  # + hidden_size
  ...
  def _time_step(time, output_ta_t, state):
    ...
    input_tdata = input_t[:, 0:i_size - 2]
    input_t_gate_bottom = input_t[:, i_size - 2]
    input_t_gate_top = input_t[:, i_size-1]
    ...
    if is_keras_rnn_cell and not nest.is_sequence(state):
      state = [state]
    # call_cell = lambda: cell(input_tdata, state)
    call_cell = lambda: cell(input_tdata, state, input_t_gate_bottom, input_t_gate_top)
