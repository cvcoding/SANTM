...
class TrainAgent(object):
    ...
    def train(self, data_path, max_gradient_norm, weight_decay, test_agent,
        ...
        model = RNN_Model(x_train[0].shape[-1], y_train.shape[-1],
                          ...
                          cell_type=args.cell,
                          chrono=args.chrono,
                          # mask_noround=self.mask_noround,
                          mse=self.mse,
        ...
        with create_sess() as sess:
            ...
            for e in range(args.epochs):
                ...
                for b_num, b_indices in enumerate(minibatch_indices):
                    ...
                    _, loss, output_probs, mask, masknoround = sess.run(
                        [model.train_opt, model.loss_nowd,
                         model.output_probs, model.mask, model.mask_noround],
                        input_feed)
                ...
                mask_sparse = tf.transpose(model.mask_noround)

                top_k = seq_lengths - n * e
                ...
                model.mask_noround = tf.transpose(tf.multiply(mask_sparse, mk))
                model.mask_noround = tf.to_float(model.mask_noround)
                ...
                if 'mnist' in args.data:
                    ...
                    if self.log_test:
                        ...
                        eval_metrics[2*len(train_tags)+1:] += calculate_metrics(
                            y_valid, np.argmax(output_probs, axis=1))
        ...
        if 'mnist' in args.data:
            test_agent.test(x_valid, y_valid, self.save_path, self.config)
