...
def random_split(n, test_frac=0.1):
    ...
    test_idx = all_idx[np.random.choice(
        n, int(np.floor(test_frac * n)), replace=False)]
        ...
        if cond1 and cond2:
            if valid_frac != 0:
                while split is None:
                    ...
                    #     0.3 * test_frac * smallest_class)
                    # cond2 = np.all(
                    #     y[test_idx, :][valid_idx, :].sum(axis=0) >=
                    #     0.3 * valid_frac * smallest_class)
                    if cond1 and cond2:
                        ...
                        print(0.3 * test_frac * smallest_class)
                        print(y[test_idx, :][valid_idx, :].sum(axis=0))
                        print(0.3 * valid_frac * smallest_class)
            else:
            ...
            print(0.35 * (1 - test_frac) * smallest_class)
            print(y[test_idx, :].sum(axis=0))
            print(0.35 * test_frac * smallest_class)
            print('Testting...')
