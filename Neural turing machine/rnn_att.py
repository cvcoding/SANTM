...
def dynamic_rnn(cell, inputs, batch_size=None, sequence_length=None, initial_state=None,
...
def _dynamic_rnn_loop(cell,
  ...
  # output_shape = state[0].shape[1]
  # state_total = tf.get_variable('state_total',
  #                                  [time_steps.eval(), output_shape])
  ...
  output_pres = tf.TensorArray(dtype=tf.float32, size=1, dynamic_size=True, clear_after_read=False)



  def _time_step(time, output_pres, output_ta_t, state):
    ...
    def f1(output_pres):
        output_t = tuple(ta.read(time-1) for ta in output_ta_t)
        output_t = nest.pack_sequence_as(structure=output_t, flat_sequence=output_t)
        output_ta_temp = output_t[0]
        output_pres = output_pres.write(time-1, output_ta_temp)
        return output_pres
    def f2():
        return output_pres

    output_pres = tf.cond(time > 0, lambda:f1(output_pres), lambda:f2())
    ...
    return (time + 1, output_pres, output_ta_t, new_state)
  ...
  _, output_pres, output_final_ta, final_state = control_flow_ops.while_loop(
      cond=lambda time, *_: time < loop_bound,
      body=_time_step,
      loop_vars=(time, output_pres, output_ta, state),
      parallel_iterations=parallel_iterations,
